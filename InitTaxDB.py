...
def setup_psqldb(sql_user,sql_db,sql_host):
	...
	con.commit()
	cur.execute("""CREATE TABLE taxonomy (id serial PRIMARY KEY, species varchar references genome_metadata(species), taxonomy_id int, lineage varchar, date_added date, date_modified date);""")
	con.commit()
	cur.close()
	con.close()
	return
